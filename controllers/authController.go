...
import (
	...
	"github.com/codergab/gojwt/database"
...
func Login(c *fiber.Ctx) error {
	...
	if err := bcrypt.CompareHashAndPassword(user.Password, []byte(data["password"])); err != nil {
		...
		return c.JSON(fiber.Map{"message": "Invalid User Credentials"})
	}
	...
	cookie := fiber.Cookie{Name: "jwt", Value: token, Expires: time.Now().Add(time.Hour * 24), HTTPOnly: true}
	c.Cookie(&cookie)
